...
class Hand(BaseModel):
    tiles: List[str] = Field(..., description="136形式の配列であり和了時の手牌です。14枚になります。例: ['1m', '2m', '3m', '4m', '4m', '5p', '5p', '5p', '7p', '8p', '9p', '1z', '1z', '1z']")
    melds: Optional[List[List[str]]] = Field(None, description="鳴きの情報。鳴きは136形式の配列であり、それらが配列になっています。例: [['1m', '2m', '3m'], ['4m', '5m', '6m']]")
    win_tile: str = Field(..., description="和了牌。例: '1m'")
    dora_indicators: Optional[List[str]] = Field(None, description="ドラ表示牌のリスト")
    is_riichi: bool = Field(False, description="リーチ宣言済みかどうか。鳴くとリーチができません。つまりmeldsがNoneでない場合は必ずFalseです。")
    ...
    player_wind: Optional[str] = Field(None, description="自風 (east, south, west, north)")
    round_wind: Optional[str] = Field(None, description="場風 (east, south, west, north)")
