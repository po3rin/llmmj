...
def convert_tiles_to_136_array(tiles: List[str]) -> List[int]:
    ...
    result = TilesConverter.string_to_136_array(man=man, pin=pin, sou=sou, honors=honors)
...
def calculate_score(hand: Hand) -> ScoreResponse:
    ...
    try:
        ...
        # 鳴きの情報を変換
        mahjong_melds = convert_melds_to_mahjong_format(hand.melds) if hand.melds else []
        logger.debug(f"Converted melds: {mahjong_melds}")
        
        # ドラ表示牌を変換
        dora_indicators = convert_tiles_to_136_array(hand.dora_indicators) if hand.dora_indicators else []
...
def validate_meld(tiles: List[str], melds: List[str]) -> bool:
    """
    鳴きの形式が正しいかチェックする
    ...
    """
    try:
        logger.debug(f"Validating meld: {meld}")
