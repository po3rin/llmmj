...
from api.mahjong_utils import calculate_score, validate_tiles
...
async def calculate(request: ScoreRequest) -> ScoreResponse:
    ...
    # ドラ表示牌の形式チェック
    if request.hand.dora_indicators and not validate_tiles(request.hand.dora_indicators):
        logger.error(f"Invalid tile format in dora indicators: {request.hand.dora_indicators}")
        raise HTTPException(status_code=400, detail="Invalid tile format in dora indicators")
    ...
    if request.melds:
        for meld in request.melds:
            if not validate_tiles(meld):
                ...
                raise HTTPException(status_code=400, detail="Invalid tile format in melds")
